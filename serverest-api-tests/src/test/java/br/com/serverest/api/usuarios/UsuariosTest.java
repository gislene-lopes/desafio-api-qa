...
public class UsuariosTest extends BaseTest {
	...
	public void criarUsuarioComSucesso() {
		...
		given().contentType("application/json").body(payloadComEmail).when().post("/usuarios").then().statusCode(201)
	...
	public void criarUsuarioEmailDuplicado() {
		...
		File payload = new File("src/test/resources/payloads/usuarioEmailDuplicado.json");

		given().contentType("application/json").body(payload).when().post("/usuarios").then().statusCode(400)
